...
pub fn task_system_chop(mut commands: Commands, mut query: Query<(Entity, &mut Brain, &Position, Option<&Targeting>), Without<Pathing>>, mut targets: Query<(Entity, &Position, &Choppable, &mut Plant), With<WorkTarget>>, workmarkers: Query<(Entity, &Parent), With<WorkMarker>>, sprite_sheet: Res<SpriteSheet>) {
    ...
    'outer: for (entity, mut brain, position, targeting) in query.iter_mut() {
        ...
        for (targetable_entity, targetable_position, _, mut plant) in targets.iter_mut() {
            ...
            if distance <= 1 && targeting.is_some() && targeting.unwrap().target == targetable_entity {
                ...
                crate::task_system::remove_x_markers(&mut commands, &workmarkers, targetable_entity);
            ...
            if already_targeted.contains(&targetable_entity) {
                continue;
            }
...
fn spawn_logs(commands: &mut Commands, targetable_entity: Entity, targetable_position: &Position, sprite_sheet: &Res<SpriteSheet>, plant: &mut Plant) {
    //plant.growth = 0.1;
    ...
    for i in 2..4 {
        ...
        commands.spawn(SpriteSheetBundle { sprite, texture_atlas: sprite_sheet.0.clone(), ..Default::default() }).insert(Logs).insert(p).insert(p.to_transform_layer(2.0)).insert(pt);
    }
}
