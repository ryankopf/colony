    ...
    'brains: for (entity, mut brain, position, targeting) in query.iter_mut() {
        ...
        'targets: for (targetable_entity, targetable_position, zone) in targetables.iter() {
            ...
            for (e, obstacle) in obstacles.iter() {
                if (obstacle == targetable_position) && (entity != e) {
                    continue 'targets;
                }
...
fn spawn_plant(commands: &mut Commands, position: &Position, sprite_sheet: &Res<SpriteSheet>, zone: &Zone) {
    // commands.entity(foragable_entity).remove::<Foragable>();
    let sprite = TextureAtlasSprite::new(zone.plant_type.sprite_index());
    commands.spawn(SpriteSheetBundle { sprite, texture_atlas: sprite_sheet.0.clone(), transform: Transform::from_xyz(position.x as f32 * TILE_SIZE, position.y as f32 * TILE_SIZE, position.z as f32 * TILE_SIZE), ..default() }).insert(*position).insert(position.to_transform_layer(0.5)).insert(Plant { growth: 0.4, plant_type: zone.plant_type });
}
