...
pub fn task_system_eat(mut commands: Commands, mut query: Query<(Entity, &mut Brain, &Position, Option<&Targeting>, Option<&mut Status>), Without<Pathing>>, query_food: Query<(Entity, &Position, &Food)>) {
    ...
    for (entity, mut brain, position, targeting, mut status) in query.iter_mut() {
        ...
        for (food_entity, food_position, _) in query_food.iter() {
            // Future food nutrition & distance calculate.
            if already_targeted.contains(&food_entity) && (targeting.is_none() || (targeting.is_some() && targeting.unwrap().target != food_entity)) {
                continue;
            }
