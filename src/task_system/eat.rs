...
pub fn task_system_eat(
    ...
    let mut already_targeted = query
        .iter()
        .filter(|(_, _, _, targeting, _)| targeting.is_some())
        .map(|(_, _, _, targeting, _)| targeting.unwrap().target)
        .collect::<Vec<Entity>>();
    for (entity, mut brain, position, targeting, mut status) in query.iter_mut() {
        ...
        for (food_entity, food_position, _) in query_food.iter() {
            ...
            if already_targeted.contains(&food_entity)
                && (targeting.is_none()
                    || (targeting.is_some() && targeting.unwrap().target != food_entity))
            {
                continue;
            }
        ...
        if let Some(closest_entity) = closest_entity {
            ...
            commands.entity(entity).insert(Pathing {
                path: vec![],
                destination: *closest_position.unwrap(),
                ..default()
            });
