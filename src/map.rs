...
pub fn _update_map_tiles(mut commands: Commands, tiles: Query<(&Position, &TileType), With<MapTile>>) {
...
fn spawn_tile(commands: &mut Commands, position: Position, tile_type: TileType, sprite_sheet: &SpriteSheet) {
    let sprite = TextureAtlasSprite::new(tile_type.to_index());
    commands.spawn(SpriteSheetBundle { sprite, texture_atlas: sprite_sheet.0.clone(), transform: Transform::from_xyz(position.x as f32 * TILE_SIZE, position.y as f32 * TILE_SIZE, position.z as f32 * TILE_SIZE), ..Default::default() }).insert(MapTile).insert(position).insert(tile_type).insert(SizeXYZ::flat(TILE_SIZE)).insert(position.to_transform());
}
